<div align="center">

<h1>Adrian Vinicius Ferreira Alves</h1>

<p>
  <a href="https://www.linkedin.com/in/adrian-alvesz/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn"/></a>
  <a href="https://byteshift.com.br"><img src="https://img.shields.io/badge/ByteShift_Atlas-0f0f1a?style=for-the-badge&logoColor=white" alt="ByteShift"/></a>
  <a href="https://tryhackme.com/p/Sanityz"><img src="https://img.shields.io/badge/TryHackMe-Top_3%25-212C42?style=for-the-badge&logo=tryhackme&logoColor=white" alt="TryHackMe"/></a>
  <a href="https://app.hackthebox.com/users/51099"><img src="https://img.shields.io/badge/Hack_The_Box-Hacker-9FEF00?style=for-the-badge&logo=hackthebox&logoColor=black" alt="HTB"/></a>
</p>

<p><strong>Red Team · Detection Engineering · SOC Platform Builder · Founder @ ByteShift Atlas</strong></p>
<p><sub>São Paulo, SP — Brasil</sub></p>

</div>

---

### About

Cybersecurity professional with hands-on experience across offensive and defensive security. Founder of **ByteShift Atlas**, a multi-tenant SaaS SOC platform consolidating SIEM, SOAR, CSPM, CWPP, CIEM and DSPM into a unified SecOps solution with MITRE ATT&CK-based detection engineering.

Previously worked as **Endpoint Security Analyst at Santander Brasil** (via Motivus), operating CrowdStrike Falcon, Netskope, Cisco Umbrella, Trellix, Qualys and Splunk in a large-scale financial environment.

Certified **CRTA (Certified Red Team Analyst)** — current focus is offensive security: red team operations, Active Directory attacks, web exploitation, vulnerability research and building tools that bridge the gap between attack and defense.

---

### Certifications

| Certification | Issuer |
|---|---|
| 🔴 CRTA — Certified Red Team Analyst | Cyber Warfare Labs |
| 🔵 SC-200 Microsoft Security Operations Analyst | Microsoft |
| ☁️ AWS Certified Cloud Practitioner | AWS · valid Aug 2026 |
| 🧩 Detection Engineering Course | SecDay · Feb 2025 |
| 🎯 Jr Penetration Tester Learning Path | TryHackMe · Aug 2024 |
| 🔵 Junior Cybersecurity Analyst Career Path | Cisco · Jan 2024 |
| 🔵 Ethical Hacker | Cisco · Jan 2024 |
| 🔵 Endpoint Security | Cisco · Aug 2024 |
| 🛡️ Cybersecurity Awareness — CAPC | Certiprof · Aug 2024 |

---

### Tech stack

**Offensive Security**

![Kali Linux](https://img.shields.io/badge/Kali_Linux-557C94?style=flat-square&logo=kalilinux&logoColor=white)
![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6633?style=flat-square&logo=portswigger&logoColor=white)
![Metasploit](https://img.shields.io/badge/Metasploit-2596CD?style=flat-square&logo=metasploit&logoColor=white)
![Nmap](https://img.shields.io/badge/Nmap-4682B4?style=flat-square&logoColor=white)
![BloodHound](https://img.shields.io/badge/BloodHound-FF0000?style=flat-square&logoColor=white)
![sqlmap](https://img.shields.io/badge/sqlmap-CC0000?style=flat-square&logoColor=white)
![Mimikatz](https://img.shields.io/badge/Mimikatz-333333?style=flat-square&logoColor=white)

**SIEM & Detection Engineering**

![Splunk](https://img.shields.io/badge/Splunk-000000?style=flat-square&logo=splunk&logoColor=white)
![Wazuh](https://img.shields.io/badge/Wazuh-3CAADE?style=flat-square&logoColor=white)
![Sentinel](https://img.shields.io/badge/Microsoft_Sentinel-0078D4?style=flat-square&logo=microsoft&logoColor=white)
![QRadar](https://img.shields.io/badge/IBM_QRadar-052FAD?style=flat-square&logo=ibm&logoColor=white)
![Elastic](https://img.shields.io/badge/Elastic-005571?style=flat-square&logo=elasticsearch&logoColor=white)

**Endpoint & Network Security**

![CrowdStrike](https://img.shields.io/badge/CrowdStrike_Falcon-E0002B?style=flat-square&logoColor=white)
![Netskope](https://img.shields.io/badge/Netskope-005B91?style=flat-square&logoColor=white)
![Cisco Umbrella](https://img.shields.io/badge/Cisco_Umbrella-049FD9?style=flat-square&logo=cisco&logoColor=white)
![Trellix](https://img.shields.io/badge/Trellix-D9232E?style=flat-square&logoColor=white)
![Qualys](https://img.shields.io/badge/Qualys-ED2025?style=flat-square&logoColor=white)

**Cloud Security**

![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![GCP](https://img.shields.io/badge/GCP-4285F4?style=flat-square&logo=googlecloud&logoColor=white)

**Languages & Automation**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=flat-square&logo=powershell&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)

**Frameworks & Standards**

![MITRE ATT&CK](https://img.shields.io/badge/MITRE_ATT%26CK-E30020?style=flat-square&logoColor=white)
![NIST](https://img.shields.io/badge/NIST-003087?style=flat-square&logoColor=white)
![CIS Controls](https://img.shields.io/badge/CIS_Controls-003087?style=flat-square&logoColor=white)
![ISO 27001](https://img.shields.io/badge/ISO_27001-003087?style=flat-square&logoColor=white)
![OWASP](https://img.shields.io/badge/OWASP-000000?style=flat-square&logo=owasp&logoColor=white)
![LGPD](https://img.shields.io/badge/LGPD-009c3b?style=flat-square&logoColor=white)

---

### Main project — ByteShift Atlas

> Multi-tenant SaaS SOC platform consolidating SIEM, SOAR, CSPM, CWPP, CIEM and DSPM into a single SecOps solution — built from scratch.

| | |
|---|---|
| **Stack** | Node.js · TypeScript · Fastify · Next.js · PostgreSQL · TimescaleDB |
| **SIEM Integrations** | Wazuh · Splunk · Microsoft Sentinel · QRadar · Elastic · CrowdStrike |
| **Detection Engine** | Custom correlation engine — behavioral similarity, sliding window counters, MITRE ATT&CK mapping, SOAR playbooks |
| **Cloud Security** | CSPM · CWPP · CIEM · DSPM — AWS, Azure, GCP |
| **AI** | Security assistant with automated IOC correlation, MITRE mapping and natural language recommendations |
| **Auth & Multi-tenancy** | mTLS · RBAC · Row-Level Security · MFA |

---

### CTF & Hacking labs

```
TryHackMe    │  Top 3% global  ·  103+ rooms  ·  Jr Penetration Tester cert
Hack The Box  │  Rank: Hacker  ·  Machines: Silentium · Kobold · CCTV · WingData · Facts
```

Skills: recon · web exploitation · API security · Linux privesc · Active Directory attacks · OSINT · lateral movement · WAF evasion

---

### Experience highlights

**ByteShift Atlas** — Founder & Security Engineer · *Dec 2025 – Present*
Building a multi-tenant SaaS SOC platform from scratch. Conducting authorized pentest engagements in Brazilian companies (web, API, infrastructure). Detection engineering, SOAR automation, cloud security and threat intelligence.

**Santander Brasil** (via Motivus) — SOC Analyst, Endpoint Security · *Apr 2024 – Jul 2025*
Operated CrowdStrike Falcon, Trellix, Netskope, Cisco Umbrella, Qualys and Splunk in a large-scale financial environment. Incident response, vulnerability management and ITSM.

---

<div align="center">
<sub>Detection is an art. Every log tells a story — you just have to know how to read it.</sub>
</div>
